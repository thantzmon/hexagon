...
def makeRow(PRN):
    ...
    try:
        ...
        f = opener.open('https://beacon.schneidercorp.com/Application.aspx?AppID=104&LayerID=1435&PageTypeID=4&PageID=805&Q=633415402&KeyValue=' + PRN)
        ...
        #find the index where the phrase "Primary Owner" followed by a new line is in all_text. The line under it should be the actual 
        #account name.
        accountNameIndex = all_text.find("Primary Owner\n")
        #using the index of the phrase "Primary owner", removes everything before it in order to make the first line "Owners"
        ...
        address1Index = all_text.find("Primary Owner\n")
...
#when you run the program you say "python grabDataMorrison.py PRNs.txt". This sets script = grabDataMorrison.py and 
#filename = PRNs.txt.
...
#sets the file to write to as "Morrison County.csv"
filename = 'Morrison County.csv'
